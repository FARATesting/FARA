...
- Enter the "StateTransitionModel" directory. ```cd "FARA tool and source code/StateTransitionModel"```
- Compile the "StateTransitionModel" module. You can use maven to compile (e.g., ```mvn compile```).
- Run the main method in the src directory (e.g., ```java -cp Main```).

### Step 2: Execute Dynamic Analysis
- Enter the "DynamicAnalysis" directory. ```cd "../DynamicAnalysis"```
- Compile the "DynamicAnalysis" directory. You can use gradle to build (e.g., ```gradlew build```).
- Run the "dynamicAnalysis" method in the "DynamicTest" class within the "app" directory.

### Step 3: Launch GUI Testing
- Navigate to the "FARA tool and source code" root directory. ```cd ../..```
- Execute the GUI testing process. ```python gui-testing.py```
